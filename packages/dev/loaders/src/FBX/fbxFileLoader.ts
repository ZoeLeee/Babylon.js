 ...
 */
export enum FBXLoaderCoordinateSystemMode {
    /**
 ...
 */
export enum FBXLoaderAnimationStartMode {
    /**
 ...
 */
export interface IFBXLoaderData {
    /**
 ...
 */
export interface IFBXLoaderExtension {
    /**
...
/** @hidden */
export interface IFBXLoader extends IDisposable {
    importMeshAsync: (
        meshesNames: any,
        scene: Scene,
    ...
    loadAsync: (scene: Scene, data: IFBXLoaderData, rootUrl: string, onProgress?: (event: ISceneLoaderProgressEvent) => void, fileName?: string) => Promise<void>;
}

/**
 * File loader for loading glTF files into a scene.
 */
export class FBXFileLoader implements IDisposable, ISceneLoaderPluginAsync, ISceneLoaderPluginFactory {
    /** @hidden */
    public static _CreateGLTF1Loader: (parent: FBXFileLoader) => IFBXLoader;

    /** @hidden */
    public static _CreateGLTF2Loader: (parent: FBXFileLoader) => IFBXLoader;
     ...
     */
    public onParsedObservable = new Observable<IFBXLoaderData>();

    private _onParsedObserver: Nullable<Observer<IFBXLoaderData>>;

    /**
     * Raised when the asset has been parsed
     */
    public set onParsed(callback: (loaderData: IFBXLoaderData) => void) {
     ...
     */
    public coordinateSystemMode = FBXLoaderCoordinateSystemMode.AUTO;

    /**
     * The animation start mode. Defaults to FIRST.
     */
    public animationStartMode = FBXLoaderAnimationStartMode.FIRST;
     ...
     */
    public readonly onExtensionLoadedObservable = new Observable<IFBXLoaderExtension>();

    private _onExtensionLoadedObserver: Nullable<Observer<IFBXLoaderExtension>>;

    /**
     * Callback raised after a loader extension is created.
     */
    public set onExtensionLoaded(callback: (extension: IFBXLoaderExtension) => void) {
    ...
    private _onValidatedObserver: Nullable<any>;

    /**
     * Callback raised after a loader extension is created.
     */
    public set onValidated(callback: (results: any) => void) {
    ...
    private _loader: Nullable<IFBXLoader> = null;
    private _state: Nullable<FBXLoaderState> = null;
    ...
    public canDirectLoad(data: string): boolean {
        return (
            ...
            StringTools.StartsWith(data, "data:base64," + FBXFileLoader._MagicBase64Encoded) || // this is technically incorrect, but will continue to support for backcompat.
            StringTools.StartsWith(data, "data:;base64," + FBXFileLoader._MagicBase64Encoded) ||
            StringTools.StartsWith(data, "data:application/octet-stream;base64," + FBXFileLoader._MagicBase64Encoded) ||
            StringTools.StartsWith(data, "data:model/gltf-binary;base64," + FBXFileLoader._MagicBase64Encoded)
        );
    }
    ...
    public directLoad(scene: Scene, data: string): Promise<any> {
        if (
            StringTools.StartsWith(data, "base64," + FBXFileLoader._MagicBase64Encoded) || // this is technically incorrect, but will continue to support for backcompat.
            StringTools.StartsWith(data, ";base64," + FBXFileLoader._MagicBase64Encoded) ||
            StringTools.StartsWith(data, "application/octet-stream;base64," + FBXFileLoader._MagicBase64Encoded) ||
            StringTools.StartsWith(data, "model/gltf-binary;base64," + FBXFileLoader._MagicBase64Encoded)
        ) {
     ...
     */
    public get loaderState(): Nullable<FBXLoaderState> {
        return this._state;
    }
     ...
     */
    public onLoaderStateChangedObservable = new Observable<Nullable<FBXLoaderState>>();
     ...
     * @hidden
     */
    public _setState(state: FBXLoaderState): void {
        ...
        this._log(FBXLoaderState[this._state]);
    }
    ...
    private _getLoader(loaderData: IFBXLoaderData): IFBXLoader {
        ...
        const version = FBXFileLoader._parseVersion(asset.version);
        if (!version) {
            throw new Error("Invalid version: " + asset.version);
        }

        if (asset.minVersion !== undefined) {
            const minVersion = FBXFileLoader._parseVersion(asset.minVersion);
            if (!minVersion) {
                throw new Error("Invalid minimum version: " + asset.minVersion);
            }

            if (FBXFileLoader._compareVersion(minVersion, { major: 2, minor: 0 }) > 0) {
                throw new Error("Incompatible minimum version: " + asset.minVersion);
            }
        }

        const createLoaders: { [key: number]: (parent: FBXFileLoader) => IFBXLoader } = {
            1: FBXFileLoader._CreateGLTF1Loader,
            2: FBXFileLoader._CreateGLTF2Loader,
        };
    ...
    private _unpackBinaryAsync(dataReader: DataReader): Promise<IFBXLoaderData> {
        ...
        return dataReader.loadAsync(20).then(() => {
            ...
            let unpacked: Promise<IFBXLoaderData>;
            switch (version) {
                case 1: {
    ...
    private _unpackBinaryV1Async(dataReader: DataReader, length: number): Promise<IFBXLoaderData> {
        const ContentFormat = {
            JSON: 0,
        };
        ...
        const data: IFBXLoaderData = { json: this._parseJson(dataReader.readString(contentLength)), bin: null };
        if (bodyLength !== 0) {
    ...
    private _unpackBinaryV2Async(dataReader: DataReader, length: number): Promise<IFBXLoaderData> {
        const ChunkFormat = {
            JSON: 0x4e4f534a,
            BIN: 0x004e4942,
        };
        ...
        return dataReader.loadAsync(chunkLength + 8).then(() => {
            const data: IFBXLoaderData = { json: this._parseJson(dataReader.readString(chunkLength)), bin: null };

            const readAsync = (): Promise<IFBXLoaderData> => {
                const chunkLength = dataReader.readUint32();
                const chunkFormat = dataReader.readUint32();
    ...
    private _logEnabled(message: string): void {
        const spaces = FBXFileLoader._logSpaces.substr(0, this._logIndentLevel * 2);
